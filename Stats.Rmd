...
removed <- removed %>%
  select(source_title, year, pub_stage, oa_status_march) %>% 
...
added <- added %>% 
  select(source_title, year, pub_stage, oa_status_april) %>% 
  arrange(source_title)
...
citations_per_paper_03 <- main %>% 
  group_by(year) %>% 
  mutate(citations_per_paper_03 = sum(citations_march)/n()) %>%   select(year, citations_per_paper_03)
