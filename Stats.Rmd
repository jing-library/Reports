...
## December
Scopus application is used to retrieve data on December 1st, 2022. 


```{r, December-data-preparation}

# read December data
scopus_december <- read_csv("data/scopus_20221201.csv")


# select only columns relevant
scopus_december <- scopus_december[c(1,3,4,5,12,13,15:17,19)]

# rename columns
names(scopus_december)[c(1:10)] <- c("author", "title", "year", "source_title", "citations_december", "DOI", "doc_type", "pub_stage", "oa_status_december", "EID")


# Replace NAs in citations and OA status, remove rows with missing or duplicated DOIs
scopus_december <- scopus_december %>% 
  replace_na(list(oa_status_december = "Closed", citations_december = 0)) %>% 
  filter(!is.na(DOI)) %>% 
  filter(!DOI %in% DOI[duplicated(DOI)]) %>% 
  write_csv("data_intermediate/scopus_december.csv")

# read November data
scopus_november <- read_csv("data_intermediate/scopus_november.csv")


# records removed from March
removed_12 <- anti_join(scopus_march, scopus_december, by = "DOI") 
removed_12 <- removed_12 %>%
  select(source_title, year, pub_stage, oa_status_march, DOI) %>% 
  arrange(source_title) %>% 
  write_csv("data_intermediate/removed_12.csv")

# new records in December
added_12 <- anti_join(scopus_december, scopus_march, by = "DOI") 

added_12 <- added_12 %>% 
  select(source_title, year, pub_stage, oa_status_december, DOI) %>%
  arrange(source_title)

# shared items of November and December
main_12 <- inner_join(scopus_november, scopus_december, by = "DOI")
main_12 <- main_12[-c(11:14, 16, 17, 19)]

names(main_12)[c(1:12)] <- c("author", "title", "year", "source_title", "citations_november", "DOI", "doc_type", "pub_stage", "oa_status_november", "EID", "citations_december", "oa_status_december")
```
* Number of records (Comparing with March)
  After removing records with missing DOIs or duplicated DOIs, **9,881 records** were kept. **60 items** were removed from the data set of March and **122 items** were added. The total data change was **around 1.9%**.
  
* OA Status (Comparing with November)
  * Comparing with November, the OA status of **109 items** changed. 16 OA items turned to be closed; while 15 items became open access. 
  * **14 items turned green where 6 of them were closed items in November**. 38 items lost their green status.

```{r, December-findings}

#---- Findings - OA Status ----

oa_status_change_12 <- main_12 %>% 
  select(title, source_title, year, oa_status_november, oa_status_december, DOI) %>%
  filter(main_12$oa_status_november != main_12$oa_status_december) %>% 
  arrange(source_title) 

open2close_12 <- oa_status_change_12 %>% 
  filter(oa_status_december == "Closed") %>% 
  write_csv("data_intermediate/open2close_12.csv")

close2open_12 <- oa_status_change_12 %>% 
...
close2green_12 <- oa_status_change_12 %>% 
  filter(oa_status_november == "Closed" & str_detect(oa_status_december, "Green")) %>% 
  write_csv("data_intermediate/close2green_12.csv")

  
green_added_12 <- oa_status_change_12 %>% 
  filter(!str_detect(oa_status_november, "Green") & str_detect(oa_status_december, "Green"))

green_removed_12 <- oa_status_change_12 %>% 
  filter(str_detect(oa_status_november, "Green") & !str_detect(oa_status_december, "Green"))
```

Publications by Year & by OA Status (Data Retrieved on December 1)
```{r, December-OA-status}

# OA status in December
main_12 %>% 
  separate(oa_status_december, c("is_oa", "oa_status", "ex_green"), sep = ", ") %>% 
...
```
* Citations
  * Comparing with November, **2,336 items** obtained more citations.
  * Citations per paper(CpP) for OA publications was **34.20** while for closed publications was **around 12.29**. 

```{r, December-citations}


# Citations in December
citations_change_12 <- main_12 %>% 
  select(title, source_title, year, citations_november, citations_december) %>% 
  filter(citations_december-citations_november != 0) %>% 
  mutate(citations_diff = citations_december - citations_november, 
         citations_percentage = ifelse(
           citations_november != 0, (citations_december - citations_november)*100 / citations_november, ifelse(citations_november == 0, NA, 999))) %>% 
  arrange(desc(citations_diff), desc(citations_percentage))

citations_error_12 <- citations_change_12 %>% 
  select(title, source_title, year, citations_november, citations_december, citations_diff) %>%
  filter(citations_diff < 0)

citations_per_paper_12 <- main_12 %>%
  separate(oa_status_december, c("is_oa", "oa_status", "ex_green"), sep = ", ") %>% 
  replace_na(list(oa_status = "Closed")) %>% 
  group_by(is_oa) %>% 
  mutate(CpP_12 = sum(citations_december)/n())
```
Citations by Year & by OA Status (Data Retrieved on December 1)

```{r, December-citations-per-paper}
...
main_12 %>% 
  ...
  mutate(citations_per_paper = round(sum(citations_december)/n())) %>%   # added a new column the citations per paper
  select(year, is_oa, oa_status, citations_december, citations_per_paper) %>%  
  ungroup() %>%  
  qhpvt(
    c("is_oa", "oa_status"), 
    "year", 
...
# shared items of October and November
...
citations_per_paper_11 <- main_11 %>%
  ...
  replace_na(list(oa_status = "Closed")) %>% 
  group_by(is_oa) %>% 
  mutate(CpP_11 = sum(citations_november)/n())
```
